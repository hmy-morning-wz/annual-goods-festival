...
App(
  Store({    
    ...
    ,async tmallResult(param){
      ...
      console.log("tmallResult",param) 
      let res = await luckyBag.tmallResult(param) 
      if(res && res.success){      
      if(this.openBag)  {
        my.navigateBack();
        setTimeout(()=>{
           this.openBag({taskId:param.taskId,code:param.code}) 
        },500)      
        
      }else {
      my.reLaunch({
        url: `/pages/index/index?taskId=${param.taskId}&code=${param.code}`
      })
      }
      }else {
      if(this.openBag)  {
       my.navigateBack();        
       setTimeout(()=>{
           this.openBag({taskId:param.taskId}) 
       },500)   
      }else {
        
        my.reLaunch({
        url: `/pages/index/index?taskId=${param.taskId}`
      });
      }
      }       
      //luckyBag.tmallResult(param)    
    },
    reLaunchDelay(url){
      setTimeout(()=>{
         my.reLaunch({
