...
createPage({
    ...
    async onLoad(options) {
        ...
        if (options.special) {
            ...
            let {taskType,sellerId,taskId} = options
            this.setData({
                url: trim(url),
                taskType,
                sellerId,
            ...
            if(taskType==='goshop') {                
                luckyBag.tmallResult({sellerId,taskType})
            }
            ...
            switch (msg.method) {
                ...
                {   
                    ...
                    break  
                      case 'tmallResult': 
                        msg.param && ( 
                            msg.param.taskType = msg.param.taskType || this.data.taskType,
                            msg.param.taskId = msg.param.taskId || this.data.taskId,
                            msg.param.sellerId = msg.param.sellerId || this.data.sellerId, 
                            app.tmallResult(msg.param))
                        ret = true
                    break            
                default:
                    ret = false
...
//scene
  this.webViewContext && app.scene && this.webViewContext.postMessage({onShow: {scene:app.scene}});
   ...
   //my.alert({content:JSON.stringify(url)});
    ...
    onBack(){
      // 页面返回时触发
      let {taskType,taskId} = this.data
      if(taskType==='goshop') {            
         if(app.openBag) {
             app.openBag({taskId,code:'SUCCESS'}) 
         }else {
            app.reLaunchDelay(`/pages/index/index?taskId=${taskId}&code=SUCCESS`)    
         }       
